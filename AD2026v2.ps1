...
if ($installMDR -eq "Yes" -or $installMDR -eq "yes" -or $installMDR -eq "Y" -or $installMDR -eq "y") {
    ...
    if (Get-Service $SnapServiceName -ErrorAction SilentlyContinue) {
        Write-Host "[$(Get-Date -f 'MM/dd/yy HH:mm:ss')] MDR is Already Installed. Skipping MDR install." -ForegroundColor Gray
        $skipMDR = $true
    }
    elseif (-not ((Get-ItemProperty 'HKLM:SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Full').Release -gt 394254)) {
        Write-Host "[$(Get-Date -f 'MM/dd/yy HH:mm:ss')] SNAP needs 4.6.1+ of .NET...SKIPPING" -ForegroundColor Yellow
        $skipMDR = $true
    }
    ...
    if (-not $skipMDR) {
        try {
            ...
            if (-not (Test-Path $InstallerPath)) {
                throw "Failed to download or file was deleted from $DownloadURL"
            }

            Start-Process -NoNewWindow -FilePath $InstallerPath -ArgumentList '-y' -Wait
            Write-Host "[$(Get-Date -f 'MM/dd/yy HH:mm:ss')] Snap Installed..." -ForegroundColor Green
        }
        catch {
            Write-Host "[$(Get-Date -f 'MM/dd/yy HH:mm:ss')] $($_.Exception.Message)" -ForegroundColor Red
            # continue
        }
    }
}

# AdvisorDefense EDR Install Step (GitHub Release Asset)

# Prompt for AdvisorDefense EDR Tool Installation
Write-Host "Do you want to install the AdvisorDefense EDR Tool for Windows? (Yes/No)" -ForegroundColor Green
$installEDR = Read-Host
if ($installEDR -eq "Yes" -or $installEDR -eq "yes" -or $installEDR -eq "Y" -or $installEDR -eq "y") {

    Write-Host "Please provide the CCID" -ForegroundColor Green
    $ccid = Read-Host

    if ([string]::IsNullOrWhiteSpace($ccid)) {
        Write-Host "CCID was empty. Skipping EDR install." -ForegroundColor Red
    }
    else {
        try {
            Write-Host "Preparing C:\ADTEMP..." -ForegroundColor Red
            $adTemp = "C:\ADTEMP"
            New-Item -ItemType Directory -Path $adTemp -Force | Out-Null

            # Save installer to ADTEMP
            $installerPath = Join-Path $adTemp "FalconSensor_Windows.exe"

            # GitHub Release Asset URL
            [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
            $edrUrl = "https://github.com/xxxVoRTeXxxx/installers/releases/download/installer/FalconSensor_Windows.exe"

            Write-Host "Downloading AdvisorDefense EDR installer to $installerPath ..." -ForegroundColor Red

            if (Test-Path $installerPath) { Remove-Item $installerPath -Force -ErrorAction SilentlyContinue }

            Invoke-WebRequest -Uri $edrUrl -OutFile $installerPath -UseBasicParsing -MaximumRedirection 10

            if (-not (Test-Path $installerPath)) {
                throw "Download failed: installer file not found at $installerPath"
            }

            # Sanity checks: not HTML + roughly the right size (your EXE is ~114MB)
            $len = (Get-Item $installerPath).Length
            if ($len -lt 80000000) {
                throw "Downloaded file is unexpectedly small ($len bytes). This usually means you downloaded an HTML page (404/repo private) instead of the EXE."
            }
            $firstByte = (Get-Content -Path $installerPath -Encoding Byte -TotalCount 1)[0]
            if ($firstByte -eq 60) { # '<'
                throw "Downloaded file appears to be HTML (starts with '<'), not the EXE. Check that the GitHub release is public and the asset URL is correct."
            }

            # Run: FILENAME /install /quiet /norestart CID=<CCID>
            Write-Host "Running AdvisorDefense EDR installer..." -ForegroundColor Red
            ...
            if ($p.ExitCode -eq 0) {
                ...
                Write-Host "AdvisorDefense EDR installer exited with code $($p.ExitCode)." -ForegroundColor Red
            }
        }
        catch {
            Write-Host "EDR install step failed: $($_.Exception.Message)" -ForegroundColor Red
        }
    }
}
else {
...
# End EDR Install Step

# Prompt for AdvisorDefense DNS Filtering Installation
Write-Host "Do you want to install the AdvisorDefense DNS Filtering solution? (Yes/No)" -ForegroundColor Green
$installDNS = Read-Host
if ($installDNS -eq "Yes" -or $installDNS -eq "yes" -or $installDNS -eq "Y" -or $installDNS -eq "y") {

    Write-Host "Please provide the SiteSecretKey" -ForegroundColor Green
    $siteSecretKey = Read-Host

    if ([string]::IsNullOrWhiteSpace($siteSecretKey)) {
        Write-Host "SiteSecretKey was empty. Skipping DNS Filtering install." -ForegroundColor Red
    }
    else {
        try {
            # Re-use ADTEMP folder (create if it doesn't exist)
            $adTemp = "C:\ADTEMP"
            New-Item -ItemType Directory -Path $adTemp -Force | Out-Null

            # Download MSI into ADTEMP
            [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
            $dnsUrl = "https://github.com/xxxVoRTeXxxx/installers/releases/download/installer/DNS_Agent_Setup.msi"
            $msiPath = Join-Path $adTemp "DNS_Agent_Setup.msi"

            Write-Host "Downloading AdvisorDefense DNS Filtering installer to $msiPath ..." -ForegroundColor Red
            if (Test-Path $msiPath) { Remove-Item $msiPath -Force -ErrorAction SilentlyContinue }
            Invoke-WebRequest -Uri $dnsUrl -OutFile $msiPath -UseBasicParsing -MaximumRedirection 10

            if (-not (Test-Path $msiPath)) {
                throw "Download failed: installer file not found at $msiPath"
            }

            # Sanity checks: not HTML + not tiny
            $len = (Get-Item $msiPath).Length
            if ($len -lt 1000000) {
                throw "Downloaded file is unexpectedly small ($len bytes). This usually means you downloaded an HTML page (404/repo private) instead of the MSI."
            }
            $firstByte = (Get-Content -Path $msiPath -Encoding Byte -TotalCount 1)[0]
            if ($firstByte -eq 60) { # '<'
                throw "Downloaded file appears to be HTML (starts with '<'), not the MSI. Check that the GitHub release is public and the asset URL is correct."
            }

            # Install silently: msiexec /qn /i "C:\path\to\DNS_Agent_Setup.msi" NKEY="SITESECRETKEY"
            Write-Host "Running AdvisorDefense DNS Filtering installer..." -ForegroundColor Red
            $msiArgs = "/qn /i `"$msiPath`" NKEY=`"$siteSecretKey`""
            $p = Start-Process -FilePath "msiexec.exe" -ArgumentList $msiArgs -Wait -PassThru

            if ($p.ExitCode -eq 0) {
                Write-Host "AdvisorDefense DNS Filtering install completed successfully." -ForegroundColor Green
            } else {
                Write-Host "AdvisorDefense DNS Filtering installer exited with code $($p.ExitCode)." -ForegroundColor Red
            }
        }
        catch {
            Write-Host "DNS Filtering install step failed: $($_.Exception.Message)" -ForegroundColor Red
        }
    }
}
else {
    Write-Host "Skipping AdvisorDefense DNS Filtering install." -ForegroundColor Gray
}
