    ...
    if (-not $skipMDR) {
        try {
            ...
            if (-not (Test-Path $InstallerPath)) {
                Write-Host "[$(Get-Date -f 'MM/dd/yy HH:mm:ss')] Failed to download or file was deleted from $DownloadURL" -ForegroundColor Red
                throw $Failure
            }
            ...
            Write-Host "[$(Get-Date -f 'MM/dd/yy HH:mm:ss')] $($_.Exception.Message)" -ForegroundColor Red
            # No return/exit here — allow script to continue to EDR step and final summary
        }
    }
}
...
if ($installEDR -eq "Yes" -or $installEDR -eq "yes" -or $installEDR -eq "Y" -or $installEDR -eq "y") {
    ...
    if ([string]::IsNullOrWhiteSpace($ccid)) {
        ...
        try {
            ...
            # Download installer into C:\ADTEMP
            [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12

            # Google Drive direct download URL (NOT the /view link)
            $edrUrl = "https://drive.google.com/uc?export=download&id=1BbR0c9gC0Q_pLJXl-2GQbOiTuBrTXoLK"

            # FILENAME saved to ADTEMP
            ...
            Invoke-WebRequest -Uri $edrUrl -OutFile $installerPath -UseBasicParsing
