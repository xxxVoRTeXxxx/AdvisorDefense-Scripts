...
# AdvisorDefense EDR Install Step
...
function Download-GoogleDriveFile {
    ...
    # First attempt (may return HTML confirmation page for large files)
    if (Test-Path $OutFile) { Remove-Item $OutFile -Force -ErrorAction SilentlyContinue }
    Invoke-WebRequest -Uri $baseUrl -WebSession $session -OutFile $OutFile -MaximumRedirection 10 -UseBasicParsing
    ...
    $looksHtml = ($firstByte -eq 60)   # '<'
    $tooSmall  = ($len -lt 5000000)    # < 5MB can't be your 114MB EXE

    if (-not $looksHtml -and -not $tooSmall) {
        return  # looks like a real binary
    }

    # Try to extract confirmation token (cookie first, then HTML parsing)
    $token = $null

    try {
        $cookies = $session.Cookies.GetCookies("https://drive.google.com")
        foreach ($c in $cookies) {
            if ($c.Name -like "download_warning*") {
                $token = $c.Value
                break
            }
        }
    ...
    if (-not $token) {
        $snippet = Get-Content -Path $OutFile -TotalCount 250 -ErrorAction SilentlyContinue | Out-String
        if ($snippet -match "confirm=([0-9A-Za-z_]+)") {
            $token = $Matches[1]
        }
    }

    if (-not $token) {
        throw "Google Drive download requires confirmation or sign-in; could not obtain confirm token (file may be restricted or quota exceeded)."
    }
    ...
    Invoke-WebRequest -Uri $confirmUrl -WebSession $session -OutFile $OutFile -MaximumRedirection 10 -UseBasicParsing

    if (-not (Test-Path $OutFile)) {
        throw "Download failed after confirmation step."
    }

    # Validate again
    $len2 = (Get-Item $OutFile).Length
    $firstByte2 = (Get-Content -Path $OutFile -Encoding Byte -TotalCount 1)[0]
    if ($firstByte2 -eq 60 -or $len2 -lt 5000000) {
        throw "Downloaded content still does not look like the installer (likely HTML/blocked). Size=$len2 bytes."
    }
}
...
if ($installEDR -eq "Yes" -or $installEDR -eq "yes" -or $installEDR -eq "Y" -or $installEDR -eq "y") {
    ...
    if ([string]::IsNullOrWhiteSpace($ccid)) {
        ...
        try {
            ...
            Download-GoogleDriveFile -FileId "1BbR0c9gC0Q_pLJXl-2GQbOiTuBrTXoLK" -OutFile $installerPath
