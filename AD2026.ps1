...
# ==============================
# AdvisorDefense EDR Install Step (Google Drive - large file safe)
# ==============================

function Download-GoogleDriveLargeFile {
    param(
    ...
    $tmp = Join-Path $outDir "_gd_tmp.html"
    if (Test-Path $tmp) { Remove-Item $tmp -Force -ErrorAction SilentlyContinue }
    if (Test-Path $OutFile) { Remove-Item $OutFile -Force -ErrorAction SilentlyContinue }

    # 1) Get the warning/confirm page (or direct download if small/public)
    Invoke-WebRequest -Uri "https://drive.google.com/uc?export=download&id=$FileId" `
        -OutFile $tmp -SessionVariable gSession -UseBasicParsing

    if (-not (Test-Path $tmp)) { throw "Failed to fetch Google Drive response page." }

    # If it actually downloaded the binary already, we'd expect it to be big and not HTML.
    # But since we wrote to tmp, we now try to extract confirm token and do the proper download.

    # 2) Extract confirm token from HTML (handles &amp; encoding)
    $confirmCode = $null
    $search = Select-String -Path $tmp -Pattern "confirm=" -AllMatches -ErrorAction SilentlyContinue
    if ($search) {
        $line = ($search | Select-Object -First 1).Line
        if ($line -match "confirm=(?<c>[^&]+)&amp;id=") { $confirmCode = $matches['c'] }
        elseif ($line -match "confirm=(?<c>[^&]+)&id=") { $confirmCode = $matches['c'] }
    }

    # Fallback: some cases accept confirm=t
    if (-not $confirmCode) { $confirmCode = "t" }

    # 3) Download the real file with confirm token (reuse cookies/session)
    Invoke-WebRequest -Uri "https://drive.google.com/uc?export=download&confirm=$confirmCode&id=$FileId" `
        -OutFile $OutFile -WebSession $gSession -MaximumRedirection 10 -UseBasicParsing

    if (-not (Test-Path $OutFile)) { throw "Download failed: installer file not created." }

    # 4) Sanity checks: not HTML + not tiny
    $len = (Get-Item $OutFile).Length
    if ($len -lt 5000000) {
        throw "Downloaded file is too small ($len bytes). Likely got an HTML block/confirm/sign-in page instead of the EXE."
    }
    $firstByte = (Get-Content -Path $OutFile -Encoding Byte -TotalCount 1)[0]
    if ($firstByte -eq 60) { # '<'
        throw "Downloaded file appears to be HTML (starts with '<'), not the EXE. File may require sign-in or download is blocked/quota-limited."
    }
...
if ($installEDR -eq "Yes" -or $installEDR -eq "yes" -or $installEDR -eq "Y" -or $installEDR -eq "y") {
    ...
    if ([string]::IsNullOrWhiteSpace($ccid)) {
        ...
        try {
            ...
            Download-GoogleDriveLargeFile -FileId "1BbR0c9gC0Q_pLJXl-2GQbOiTuBrTXoLK" -OutFile $installerPath
